...
{
    QParallelAnimationGroup *group = new QParallelAnimationGroup(boat);

    QPropertyAnimation *fadeBoat = new QPropertyAnimation(boat, "opacity", group);
    fadeBoat->setEndValue(0);
    fadeBoat->setDuration(1000);

    QSequentialAnimationGroup *explosion = new QSequentialAnimationGroup(group);
    for (int i = 1; i <= 4; i++) {
        PixmapItem *step = new PixmapItem(QString("explosion/boat/step%1").arg(i), QRect(0, 0, 1024, 768), boat);
        step->setZValue(6);
        step->setOpacity(0);
...
Boat::Boat() : PixmapItem(QString("boat"),  QRect(0, 0, 1024, 768)),
    speed(0), bombsAlreadyLaunched(0), direction(Boat::None), movementAnimation(0)
{
    ...
    ManualTransition *fireFromGraphicsView = new ManualTransition(launchStateRight);
    moving->addTransition(fireFromGraphicsView);
...
qreal Boat::currentSpeed() const
{
    return speed;
}

void Boat::setCurrentSpeed(qreal speed)
{
    if (speed > 5 || speed < 0) {
        return;
    }
