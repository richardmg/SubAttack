...
Q_OBJECT
public:
    explicit MovementState(SubMarine *submarine, QState *parent = 0) : QAnimationState(parent), submarine(submarine)
    {
...
protected slots:
    void onAnimationMovementValueChanged(const QVariant &)
    {
        ...
        if (boatVisible && qrand() % 200 + 1 == 3) {
            submarine->launchTorpedo((qrand() % 3) + submarine->torpedoSpeed);
        }
    }
